...
for j in range(num_frames):
    print('SR %d frame' % j)
    n_back = min(maxnbf, j - 1)
    n_for = min(maxnbf, num_frames - j)
    theta = np.ones((n_for + n_back, 1))

    # coordinate descent algorithm
    I = I_init[j]
    J0 = J[j]
    for i in range(-n_back - 1, n_for):  # TO_DO: check if consistent
        FI[j + i] = I_init[j + i]

    for k in range(max_iteration):
        print('%d h iteration for frame %d...\n' % (k, j))
        I_old_out = I
        ...
        if motion_estimation and optical_flow:
            ...
            for i in range(-n_back - 1, n_for):
                if i == 0:
                    ...
                    u[j + i], v[j + 1] = optical_flow_calculation.calculation(I, I_sr[j + i])
                    ut[j + i] = -u[j + i]
                    vt[j + i] = -v[j + i]
        ...
        if blur_estimation:
            ...
            for m in range(max_iteration - 2):
                ...
                if diff_K < eps_blur:
                    ...
                    break
            half = hsize // 2
            h_2d = K[high_res_height / 2 - half + 1:high_res_height / 2 + half + 1,
                   low_res_width / 2 - half + 1:low_res_width / 2 + half + 1] # TO-DO: check again if of same meaning
...
video_result = frame_utility.cconv2dt(I_sr, ycrcb_h_bicubic)
if degrad == 0:
